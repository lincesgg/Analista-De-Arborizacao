...
def return_initial_mask(base_img_path, initial_mask_image_path, should_analyze_just_the_biggest_shape):
    pixel_areas = []
    ...
    ret, thresh = cv.threshold(diff, 253, 255, cv.THRESH_BINARY)
    ...
    if should_analyze_just_the_biggest_shape:
        for contour in contours:
            for point in contour:
                ...
                while last_pixels_contaminated != []:
                    cv.imshow("contam", contamination_reference)
                    cv.waitKey(1)
        ...
        final_mask = contamination_reference

    final_mask_contour = cv.bitwise_and(final_mask, inverse_thresh)
    final_mask = cv.bitwise_xor(final_mask, final_mask_contour)
    mask_pixel_amount = cv.calcHist([final_mask], [0], None, [256], [0, 256])[255][0]
